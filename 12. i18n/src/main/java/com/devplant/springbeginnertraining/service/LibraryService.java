...
public class LibraryService {
	...
	public Lending lendBook(Book book, String clientName, LocalDate dueDate) {
		...
		if (clientName == null || clientName.isEmpty())
			throw new IllegalArgumentException(messages.getMessage("library.error.clientName"));
		if (dueDate == null || dueDate.isBefore(LocalDate.now()))
			throw new IllegalArgumentException(messages.getMessage("library.error.dueDate"));

		// check if the book exists; use the repo one
		Book bookEntity = booksService.getOne(book.getId());
		if (bookEntity == null)
			throw new IllegalArgumentException(messages.getMessage("library.error.missingBookId", book.getId()));
		...
		if (lib.getStocks().get(bookEntity) <= 0)
			throw new RuntimeException(messages.getMessage("library.error.stockDepleted", book.getTitle()));
	...
	public void returnBook(long lendingId) {
		...
		if (lending == null)
			throw new IllegalArgumentException(messages.getMessage("library.error.lendingId", lendingId));
		if (lending.getBook() == null)
			throw new IllegalArgumentException(messages.getMessage("library.error.lending.noBook"));
		if (lending.getLibrary() == null)
			throw new IllegalArgumentException(messages.getMessage("library.error.lending.noLibrary"));
