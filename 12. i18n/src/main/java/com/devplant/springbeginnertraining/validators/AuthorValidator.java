...
public class AuthorValidator {
	...
	public void checkName(Author author) {
		if (author.getName() == null || author.getName().isEmpty())
			throw new IllegalArgumentException(messages.getMessage("author.error.name.missing"));
		if (author.getName().length() < 2)
			throw new IllegalArgumentException(messages.getMessage("author.error.name.min2"));
		if (author.getName().length() > 128)
			throw new IllegalArgumentException(messages.getMessage("author.error.name.max128"));
	}

	public void checkBio(Author author) {
		if (author.getBio() == null || author.getBio().isEmpty())
			throw new IllegalArgumentException(messages.getMessage("author.error.bio.missing"));
	}
}
